...
public class MyTreeMap<Key extends Comparable<Key>, Value> {
    ...
    private boolean isBalanced(Node node) {
        if (node.height == 1 || node.height == 0) return true;
        if (node.height > 1 && (node.left == null || node.right == null)) return false;
        Node tmpRight = node.right;
        Node tmpLeft = node.left;
        if (Math.abs(tmpRight.height - tmpLeft.height) > 1) return false;
